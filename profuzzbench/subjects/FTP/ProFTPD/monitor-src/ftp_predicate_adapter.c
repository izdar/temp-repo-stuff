...
static void infer_command_from_response(int resp_code) {
    // If command is not set, try to infer from response code
    if (strcmp(g_ftp_state.last_command, "cmdNotSet") != 0) {
        return;  // Command already set
    }
    
    switch (resp_code) {
        case 220:  // Welcome or REIN
            if (g_ftp_state.sequence_number == 0) {
                // First message, no command
            } else {
                g_ftp_state.last_command[0] = '\0';
        ...
        // These could be many different commands
        default:
            break;
    }
}
