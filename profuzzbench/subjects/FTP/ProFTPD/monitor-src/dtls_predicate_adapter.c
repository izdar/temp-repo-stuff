...
static void append_packet_trace(char *out, size_t out_sz,
                                ...
                                unsigned int len,
                                const char *req_enum,
                                const char *resp_enum)
{
    g_msg_id++;
    
    // Determine direction
    const char *direction = "-";
    if (strcmp(req_enum, "requestNotSet") != 0) {
        direction = "C2S";
    } else if (strcmp(resp_enum, "responseNotSet") != 0) {
        direction = "S2C";
    }
                          ...
                          size_t out_sz)
{
    ...
    bool mac_ok = true;
    if (encrypted) {
        // If couldn't parse message type, MAC likely failed
        if (content_type == DTLS_CT_HANDSHAKE && message_type == 0xFF) {
            mac_ok = false;
        }
    ...
    snprintf(out, out_sz,
             ...
             alert_level,
             alert_description);
     append_packet_trace(out, out_sz, buf, len, req_enum, resp_enum);
}