...
/* ADD THIS LINE BELOW THE INCLUDE */
// KHASH_MAP_INIT_INT(32, uint32_t) 

monitor_handle_t *g_monitor = NULL;
int g_monitor_initialized = 0;
...
region_t* extract_requests_ssh(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if (mem_count >= 6) {
      if (!strncmp(mem, "SSH-", 4)) {
        ...
        // CRITICAL FIX: Validate before creating region
        if (cur_start > cur_end || cur_end >= buf_size) {
            if (cur_end >= buf_size) cur_end = buf_size - 1;
        ...
        /* NEW: feed this request region to the evaluator */
        if (g_monitor_initialized && g_monitor ) {
          unsigned int msg_start = regions[region_count - 1].start_byte;
          unsigned int msg_end   = regions[region_count - 1].end_byte;
          if (msg_end >= msg_start && msg_end < buf_size) {
            unsigned int msg_len = msg_end - msg_start + 1;
            char pred_line[512];

            ssh_build_request_pred_line(buf + msg_start, msg_len,
        ...
        if (cur_end < buf_size - 1) {
          ...
          // CRITICAL: We're at the end, don't try to process more
          break;
        }
      } else {
        //It could be a normal message
        ...
        // CRITICAL: Complete validation
        if (message_size < 2 || message_size > buf_size || message_size > (1 << 24)) {
            // Create region for what we have and bail out
            if (cur_start < buf_size && byte_count > cur_start) {
                region_count++;
                regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
                regions[region_count - 1].start_byte = cur_start;
                regions[region_count - 1].end_byte = (byte_count - 1 < buf_size) ? byte_count - 1 : buf_size - 1;
                regions[region_count - 1].state_sequence = NULL;
                regions[region_count - 1].state_count = 0;
            }
            break;
        ...
        // Check if we need to add MAC bytes
        if ((message_code < 20) || (message_code > 49)) {
            // Check for overflow BEFORE adding
            if (bytes_to_skip > 0xFFFFFFF7) { // Would overflow if we add 8
                if (cur_start < buf_size) {
                    region_count++;
                    regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
                    regions[region_count - 1].start_byte = cur_start;
        ...
        // Validate bytes_to_skip
        if (bytes_to_skip > buf_size - byte_count) {
            if (cur_start < buf_size) {
                region_count++;
                regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
                regions[region_count - 1].start_byte = cur_start;
        ...
        // CRITICAL: Adjust if we went past the end
        if (byte_count >= buf_size) {
            byte_count = buf_size;
            cur_end = buf_size - 1;
      ...
      //Check if the last byte has been reached
      if (cur_end >= buf_size - 1) {
        // CRITICAL: Validate before creating final region
        if (cur_end >= buf_size) cur_end = buf_size - 1;
        if (cur_start <= cur_end && cur_start < buf_size) {
            region_count++;
            regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
            regions[region_count - 1].start_byte = cur_start;
            regions[region_count - 1].end_byte = cur_end;
            ...
            /* NEW: partial tail region → also feed it */
            if (g_monitor_initialized && g_monitor) {
              unsigned int msg_start = regions[region_count - 1].start_byte;
              unsigned int msg_end   = regions[region_count - 1].end_byte;
              if (msg_end >= msg_start && msg_end < buf_size) {
  ...
  if ((region_count == 0) && (buf_size > 0)) {
    regions = (region_t *)ck_realloc(regions, sizeof(region_t));
    regions[0].start_byte = 0;
    regions[0].end_byte = (buf_size > 0) ? buf_size - 1 : 0;
    regions[0].state_sequence = NULL;
    regions[0].state_count = 0;

    region_count = 1;

    /* NEW: fallback whole-buffer region → feed once */
    if (g_monitor_initialized && g_monitor) {
      unsigned int msg_start = regions[0].start_byte;
      unsigned int msg_end   = regions[0].end_byte;
      if (msg_end >= msg_start && msg_end < buf_size) {
        unsigned int msg_len = msg_end - msg_start + 1;
        char pred_line[512];
...
region_t* extract_requests_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  // Add safety check at the very beginning
  if (buf_size == 0) {
    *region_count_ref = 0;
    return NULL;
  }
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 3) && (memcmp(&mem[mem_count - 3], terminator, 4) == 0)) {
      ...
      // CRITICAL: Validate before creating region
      if (cur_start > cur_end || cur_end >= buf_size) {
        if (cur_end >= buf_size) cur_end = buf_size - 1;
        if (cur_start > cur_end) {
          mem_count = 0;
      ...
      if (g_monitor_initialized && g_monitor) {
        ...
        if (msg_end >= msg_start && msg_end < buf_size) {
          unsigned int msg_len = msg_end - msg_start + 1;
          char pred_line[1024];  // RTSP messages can be longer

          rtsp_build_request_pred_line(buf + msg_start, msg_len,
                                      pred_line, sizeof(pred_line));
          monitor_emit_line(g_monitor, pred_line);
        }
      }
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 1) && (memcmp(&mem[mem_count - 1], terminator, 1) == 0) &&
      ...
      if (g_monitor_initialized && g_monitor) {
        ...
        if (msg_end >= msg_start && msg_end < buf_size) {
          unsigned int msg_len = msg_end - msg_start + 1;
          char pred_line[2048];  // SIP messages can be longer

          sip_build_request_pred_line(buf + msg_start, msg_len,
                                      pred_line, sizeof(pred_line));
          monitor_emit_line(g_monitor, pred_line);
        }
      }
                                         ...
                                         unsigned int* state_count_ref)
{
    ...
    if (!state_sequence) PFATAL("Unable realloc a memory region to store state sequence");
    state_sequence[state_count - 1] = 0;

    while (byte_count < buf_size) {

        unsigned int cur_start = byte_count;

        // Need at least 6 bytes to continue
        if (byte_count + 6 > buf_size) break;
        
        memcpy(mem, buf + byte_count, 6);
        mem[6] = '\0';
        byte_count += 6;

        /* Identification message: "SSH-..." */
        if (strstr(mem, "SSH")) {
            char tmp = 0x00;
            ...
            unsigned int cur_end = (byte_count > 0) ? byte_count - 1 : 0;
            ...
            if (g_monitor_initialized && g_monitor) {
                const unsigned char *msg = buf + cur_start;
                unsigned int msg_len = cur_end - cur_start + 1;

                if (msg_len > 0 && msg_len <= buf_size - cur_start) {
                    char pred_line[512];
                    ssh_build_response_pred_line(msg, msg_len,
                                                 pred_line, sizeof(pred_line));
                    monitor_emit_line(g_monitor, pred_line);
                }
            }
            ...
            // Critical: Validate message_size to prevent underflow
            if (message_size < 2) {
                break;  // Invalid message size (would cause underflow)
            }
            
            // Check if message_size is reasonable
            if (message_size > buf_size || message_size > (1 << 24)) { // 16MB max
                break;  // Message size too large
            }

            unsigned char message_code = (unsigned char)mem[5];

            // Calculate safe boundaries
            unsigned int payload_size = message_size - 2;
            
            // Check if we have enough buffer left
            if (payload_size > buf_size - byte_count) {
                break; // Not enough data left
            }
            
            unsigned int cur_end = byte_count + payload_size - 1;
            ...
            // Advance byte_count based on message type
            unsigned int bytes_to_advance = payload_size;
            
            // For non-key-exchange messages, add MAC size
            if (!(message_code >= 20 && message_code <= 49)) {
                // First check if adding 8 would overflow
                if (bytes_to_advance > buf_size - 8) {
            ...
            if (bytes_to_advance > buf_size - byte_count) {
                break; // Can't advance safely
            }
            
            byte_count += bytes_to_advance;
        }
    }
...
unsigned int* extract_response_codes_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    unsigned int cur_start = byte_count - mem_count;  // Track message start
    ...
    if ((mem_count > 0) && (memcmp(&mem[mem_count - 1], terminator, 2) == 0)) {
      if ((mem_count >= 5) && (memcmp(mem, rtsp, 5) == 0)) {
        //Extract the response code which is the first 3 bytes after "RTSP/1.0 "
        
        // Validate we have enough bytes for status code
        if (mem_count < 13) {  // "RTSP/1.0 XXX\r\n" minimum
          mem_count = 0;
          continue;
        }
        ...
        unsigned int cur_end = byte_count - 1;
        ...
        if (g_monitor_initialized && g_monitor) {
          ...
          if (msg_len > 0 && msg_len <= buf_size - cur_start) {
            char pred_line[1024];  // RTSP responses can be long
            rtsp_build_response_pred_line(msg, msg_len,
                                         pred_line, sizeof(pred_line));
            monitor_emit_line(g_monitor, pred_line);
          }
        }
...
unsigned int* extract_response_codes_sip(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 0) && (memcmp(&mem[mem_count - 1], terminator, 2) == 0)) {
      if ((mem_count >= 4) && (memcmp(mem, sip, 4) == 0)) {
        //Extract the response code which is the first 3 bytes after "SIP/2.0 "
        
        // Validate we have enough bytes for status code
        if (mem_count < 12) {  // "SIP/2.0 XXX\r\n" minimum
          mem_count = 0;
          continue;
        }
        ...
        if (g_monitor_initialized && g_monitor) {
          ...
          if (msg_len > 0 && msg_len <= buf_size - cur_start) {
            char pred_line[2048];  // SIP responses can be long
            sip_build_response_pred_line(msg, msg_len,
                                        pred_line, sizeof(pred_line));
            monitor_emit_line(g_monitor, pred_line);
          }
        }
