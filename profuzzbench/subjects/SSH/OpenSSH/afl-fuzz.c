...
extern monitor_handle_t *g_monitor;
extern int g_monitor_initialized;
...
int send_over_network()
{
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    if (g_monitor_initialized && g_monitor) {
        char pred_line[2048];
        u32 resp_len = response_buf_size - prev_buf_size;
        build_response_pred_line(response_buf + prev_buf_size, resp_len,
                                      pred_line, sizeof(pred_line));
        monitor_emit_line(g_monitor, pred_line);
    }
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  // NEW: Check for violation before returning
...
keep_as_crash:
      ...
      break;
    // In afl-fuzz.c, FAULT_VIOLATION case (around line 4165):
  ...
  case FAULT_VIOLATION:
    ...
    fd = open(fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
    if (fd >= 0 && state_sequence && state_count > 0) {
        ck_write(fd, state_sequence, state_count * sizeof(unsigned int), fn);
        close(fd);
    }
    ck_free(fn);
    ...
    if (meta_file) {
        ...
        fprintf(meta_file, "State Count: %u\n", state_count);
        fprintf(meta_file, "\n");
        
        // Add state sequence in readable form
        fprintf(meta_file, "State Sequence:\n");
        if (state_sequence && state_count > 0) {
            for (unsigned int i = 0; i < state_count; i++) {
                fprintf(meta_file, "  [%u] = %u\n", i, state_sequence[i]);
            }
        }
        fprintf(meta_file, "\n");
...
static u8 fuzz_one(char** argv) {
  ...
  if (g_monitor_initialized) {
    /* start a fresh logical session for this test case */
...
havoc_stage:
  ...
  if (g_monitor_initialized && g_monitor) {
    monitor_end_session(g_monitor);  /* close LTL trace for this input */
  }
  new_hit_cnt = queued_paths + unique_crashes;
...
void shutdown_net_monitor(void) {
  if (g_monitor_initialized) {
...
EXP_ST void setup_dirs_fds(void) {
  ...
  memset(virgin_violations, 255, MAP_SIZE);
  total_violations = 0;
  unique_violations = 0;
  last_violation_time = 0;
  
  // Create all violation directories
  u8* vdir;
  
  vdir = alloc_printf("%s/violations", out_dir);
  if (mkdir(vdir, 0700)) {
      if (errno != EEXIST) PFATAL("Unable to create '%s'", vdir);
  }
  ck_free(vdir);
  
  vdir = alloc_printf("%s/replayable-violations", out_dir);
  if (mkdir(vdir, 0700)) {
      if (errno != EEXIST) PFATAL("Unable to create '%s'", vdir);
  }
  ck_free(vdir);
  
  vdir = alloc_printf("%s/violations-metadata", out_dir);
  if (mkdir(vdir, 0700)) {
      if (errno != EEXIST) PFATAL("Unable to create '%s'", vdir);
  }
  ck_free(vdir);
  
  vdir = alloc_printf("%s/violations-traces", out_dir);
  if (mkdir(vdir, 0700)) {
      if (errno != EEXIST) PFATAL("Unable to create '%s'", vdir);
  }
  ck_free(vdir);
...
int main(int argc, char** argv) {
      ...
      case 'P': /* protocol to be tested */
        ...
        if (!strcmp(optarg, "RTSP")) {
          if (!g_monitor_initialized) {
            ...
            const char * eval_path = "/usr/local/bin/formula_parser";
            const char * spec_path = "/home/ubuntu/monitor/rtsp.txt";
          ...
          build_request_pred_line = &rtsp_build_request_pred_line;
          build_response_pred_line = &rtsp_build_response_pred_line;
          predicate_reset_session = &rtsp_predicate_reset_session;
           ...
           if (!g_monitor_initialized) {
            ...
            const char* eval_path = "/usr/local/bin/formula_parser";
            const char* spec_path = "/home/ubuntu/monitor/ftp.txt";
          ...
          if (!g_monitor_initialized) {
            const char * eval_path = "/usr/local/bin/formula_parser";
            const char * spec_path = "/home/ubuntu/monitor/dtls.ltl";
          ...
          if (!g_monitor_initialized) {
            const char *eval_path = "/usr/local/bin/formula_parser";
            const char *spec_path = "/home/ubuntu/monitor/dnsmasq.ltl";
          ...
          if (!g_monitor_initialized) {
            ...
            const char* eval_path = "/usr/local/bin/formula_parser";
            const char* spec_path = "/home/ubuntu/monitor/ssh-refined.txt";
          ...
          build_request_pred_line = &ssh_build_request_pred_line;
          build_response_pred_line = &ssh_build_response_pred_line;
          predicate_reset_session = &ssh_predicate_reset_session;
  ...
  if (g_monitor_initialized && g_monitor) {
    int status = monitor_stop(g_monitor);
    OKF("LTL evaluator exited with status %d", status);
    g_monitor = NULL;
    g_monitor_initialized = 0;
  }
  write_bitmap();
