...
// External monitor state (defined in afl-fuzz.c)
monitor_handle_t *g_monitor;
int g_monitor_initialized;
...
region_t* extract_requests_ssh(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if (mem_count >= 6) {
      if (!strncmp(mem, "SSH-", 4)) {
        ...
        /* ===== SSH MONITOR INTEGRATION ===== */
        if (g_monitor_initialized && g_monitor && cur_start <= cur_end) {
          unsigned int region_len = cur_end - cur_start + 1;
          char pred_line[512];
          ssh_build_request_pred_line(buf + cur_start, region_len, pred_line, sizeof(pred_line));
  ...
  if ((region_count == 0) && (buf_size > 0)) {
    ...
    if (g_monitor_initialized && g_monitor) {
      char pred_line[512];
      ssh_build_request_pred_line(buf, buf_size, pred_line, sizeof(pred_line));
      monitor_emit_line(g_monitor, pred_line);
    }
    /* ===== END MONITOR ===== */
...
region_t* extract_requests_dns(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  for (unsigned int byte_count = 0; byte_count < buf_size; byte_count++) {
    ...
    if ((mem_count >= 12) && (*(mem+mem_count) == 0)) {
      ...
      /* ===== DNS MONITOR INTEGRATION ===== */
      if (g_monitor_initialized && g_monitor && cur_start <= cur_end) {
        unsigned int region_len = cur_end - cur_start + 1;
        char pred_line[2048];
        dnsmasq_build_pred_line(buf + cur_start, region_len, pred_line, sizeof(pred_line));
        monitor_emit_line(g_monitor, pred_line);
      }
      /* ===== END MONITOR ===== */
  ...
  if ((region_count == 0) && (buf_size > 0)) {
    ...
    if (g_monitor_initialized && g_monitor) {
      char pred_line[2048];
      dnsmasq_build_pred_line(buf, buf_size, pred_line, sizeof(pred_line));
      monitor_emit_line(g_monitor, pred_line);
    }
    /* ===== END MONITOR ===== */
...
// extract_requests_sip() - UPDATED with monitor integration
...
region_t* extract_requests_sip(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 1) && (memcmp(&mem[mem_count - 1], terminator, 1) == 0) &&
      ...
      // ============================================================
      // MONITOR INTEGRATION: Emit SIP request predicate
      // ============================================================
      if (g_monitor_initialized && g_monitor) {
        char pred_line[2048];
        unsigned int req_len = cur_end - cur_start + 1;
        
        // Safety check
        if (req_len > 0 && cur_start < buf_size && cur_end < buf_size) {
          sip_build_request_pred_line(buf + cur_start, req_len, 
      ...
      if (cur_end == buf_size - 1) {
        ...
        // ============================================================
        // MONITOR INTEGRATION: Emit final request
        // ============================================================
        if (g_monitor_initialized && g_monitor) {
          char pred_line[2048];
          unsigned int req_len = cur_end - cur_start + 1;
  ...
  if ((region_count == 0) && (buf_size > 0)) {
    ...
    // ============================================================
    // MONITOR INTEGRATION: Emit malformed request
    // ============================================================
    if (g_monitor_initialized && g_monitor) {
      char pred_line[2048];
      sip_build_request_pred_line(buf, buf_size, 
                                  pred_line, sizeof(pred_line));
      monitor_emit_line(g_monitor, pred_line);
    }
    // ============================================================
...
unsigned int* extract_response_codes_ssh(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
   ...
   while (byte_count < buf_size) {
      ...
      if (strstr(mem, "SSH")) {
        //Read until \x0D\x0A
        unsigned int start_pos = byte_count - 6;
        char tmp = 0x00;
        while (tmp != 0x0A) {
        ...
        /* ===== SSH MONITOR INTEGRATION ===== */
        if (g_monitor_initialized && g_monitor) {
          unsigned int msg_len = byte_count - start_pos;
          char pred_line[512];
          ssh_build_response_pred_line(buf + start_pos, msg_len, pred_line, sizeof(pred_line));
          monitor_emit_line(g_monitor, pred_line);
        }
        /* ===== END MONITOR ===== */
        ...
        /* ===== SSH MONITOR INTEGRATION ===== */
        if (g_monitor_initialized && g_monitor) {
          unsigned int msg_len = message_size + 4; // 4 bytes for packet_length field
          if (start_pos + msg_len <= buf_size) {
            char pred_line[512];
            ssh_build_response_pred_line(buf + start_pos, msg_len, pred_line, sizeof(pred_line));
            monitor_emit_line(g_monitor, pred_line);
          }
...
{
  /* ===== DNS MONITOR INTEGRATION START ===== */
  // Parse the entire response buffer once for monitoring
  if (g_monitor_initialized && g_monitor && buf_size > 0) {
    // Process the buffer to find individual DNS messages
    unsigned int offset = 0;
    while (offset < buf_size) {
      // Check if we have at least a DNS header (12 bytes)
      if (buf_size - offset >= 12) {
        // Try to determine message length
        // For now, process as much as we can see
        unsigned int remaining = buf_size - offset;
        char pred_line[2048];
        dnsmasq_build_pred_line(buf + offset, remaining, pred_line, sizeof(pred_line));
        monitor_emit_line(g_monitor, pred_line);
        
        // Try to skip to next message if possible
        // Look for the null terminator in the question section
        unsigned int scan_offset = offset + 12;
        bool found_null = false;
        while (scan_offset < buf_size && scan_offset < offset + remaining) {
          if (buf[scan_offset] == 0) {
            // Found end of domain name, skip QTYPE and QCLASS (4 bytes)
            offset = scan_offset + 5;
            found_null = true;
            break;
          }
          scan_offset++;
        }
        if (!found_null) break; // Can't parse further
      } else {
        break; // Not enough data for another message
      }
    }
  }
  /* ===== DNS MONITOR INTEGRATION END ===== */
...
region_t *extract_requests_dtls12(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref) {
  ...
  unsigned int cur_start = 0;
   ...
   while (byte_count < buf_size) {
     ...
     (memcmp(&buf[byte_count+1], dtls12_version, 2) == 0)) {
       ...
       // Send the previous record to monitor before starting new region
       if (g_monitor_initialized && g_monitor && cur_start < byte_count) {
         unsigned int record_len = byte_count - cur_start;
         
         // Make sure we have at least a DTLS header (13 bytes)
         if (record_len >= 13) {
           char pred_line[512];
           
           dtls_build_pred_line(buf + cur_start, 
                               record_len,
                               pred_line, 
                               sizeof(pred_line));
      ...
      if (byte_count == buf_size - 1) {
        ...
        /* ===== MONITOR INTEGRATION START ===== */
        // Send the final record to monitor
        if (g_monitor_initialized && g_monitor && cur_start <= byte_count) {
          unsigned int record_len = byte_count - cur_start + 1;
  ...
  if ((region_count == 0) && (buf_size > 0)) {
    ...
    /* ===== MONITOR INTEGRATION START ===== */
    // Even if structure is broken, try to send to monitor
    if (g_monitor_initialized && g_monitor) {
      char pred_line[512];
      
      // Try to parse whatever we have
      dtls_build_pred_line(buf, 
                          buf_size,
                          pred_line, 
                          sizeof(pred_line));
...
unsigned int* extract_response_codes_dtls12(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    (memcmp(&buf[byte_count+1], dtls12_version, 2) == 0)) {
      ...
      /* ===== MONITOR INTEGRATION START ===== */
      if (g_monitor_initialized && g_monitor) {
        // Track the start of this record
        unsigned int record_start = byte_count;
        unsigned int record_total_len = 13 + record_length;
        
        // Ensure we don't exceed buffer bounds
        if (record_start + record_total_len <= buf_size) {
          char pred_line[512];
          
          // Build predicate line for this DTLS record
          dtls_build_pred_line(buf + record_start, 
                              record_total_len,
                              pred_line, 
                              sizeof(pred_line));
...
// extract_response_codes_sip() - UPDATED with monitor integration (CORRECTED)
...
unsigned int* extract_response_codes_sip(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  unsigned int state_count = 0;
  char terminator[2] = {0x0D, 0x0A};
  char sip[4] = {0x53, 0x49, 0x50, 0x2f}; // "SIP/"
  ...
  unsigned int resp_start = 0; // Track start of current response
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 0) && (memcmp(&mem[mem_count - 1], terminator, 2) == 0)) {
      if ((mem_count >= 4) && (memcmp(mem, sip, 4) == 0)) {
        // Extract the response code which is at position 8-11 (after "SIP/2.0 ")
        char temp[4];
        memcpy(temp, &mem[8], 4);
        temp[3] = 0x0;
        unsigned int message_code = (unsigned int) atoi(temp);
        ...
        // ============================================================
        // MONITOR INTEGRATION: Emit SIP response predicate
        // ============================================================
        if (g_monitor_initialized && g_monitor) {
          char pred_line[2048];
          unsigned int resp_len = byte_count - resp_start;
          
          // Safety checks
          if (resp_len > 0 && resp_start < buf_size && 
              (resp_start + resp_len) <= buf_size) {
            sip_build_response_pred_line(buf + resp_start, resp_len, 
                                        pred_line, sizeof(pred_line));
            monitor_emit_line(g_monitor, pred_line);
          }
        }
        ...
        mem_count = 0;
        resp_start = byte_count; // Update start for next response
...
// kl_messages_mx manipulating functions
//flag = 1:get M1 ; flag = 2:get M2_M3
klist_t(lms) *construct_kl_messages_mx(klist_t(lms) *kl_messages, kliter_t(lms) *M2_prev, u8 flag)
{
  klist_t(lms) *kl_messages_res = kl_init(lms);
  kliter_t(lms) *it_res;

  if(flag == 1) {

  for (it_res = kl_begin(kl_messages); it_res != kl_next(M2_prev); it_res = kl_next(it_res)) {

    //Create a new message
    message_t *m = (message_t *) ck_alloc(sizeof(message_t));
    m->mdata = (char *) ck_alloc(kl_val(it_res)->msize);
    m->msize = kl_val(it_res)->msize;
    if (m->mdata == NULL) PFATAL("Unable to allocate memory region to store new message");
    memcpy(m->mdata, kl_val(it_res)->mdata, kl_val(it_res)->msize);

    //Insert the message to the linked list
    *kl_pushp(lms, kl_messages_res) = m;
  ...
  for (it_res = kl_next(M2_prev); it_res != kl_end(kl_messages); it_res = kl_next(it_res)) {
  ...
  else PFATAL("Not defined value");

  return kl_messages_res;
...
POLL_AGAIN:  
  ...
  if (rv > 0) {
    ...
    if ((rv < 0) && (errno != EINTR)) // an error was returned
      return 1;
    else 
    if((rv < 0) && (errno == EINTR)) 
    {
      // ACTF("Poll again 1741");
      goto POLL_AGAIN; 
    }
