...
from monitor_bridge import MonitorPool, stream_round_to_monitor, results_root_for_mode
...
class TestUnit:
	def __init__(self, unit_num:int, dnstap_interval:int = 10):
		...
		try:
			self._monitor_pool = MonitorPool()
		except Exception:
			self._monitor_pool = None

		# infer mode from the runner script name (fallback to 'cdns')
		script = os.path.basename(sys.argv[0]).lower()
		if "cdns_fallback" in script:
			self.mode = "cdns_fallback"
		elif "fwd_global" in script:
			self.mode = "fwd_global"
		elif "recursive" in script:
			self.mode = "recursive"
		else:
			self.mode = "cdns"

		# results root based on mode (results_root_for_mode returns Path)
		try:
			self._results_root = results_root_for_mode(self.mode)
		except Exception:
			self._results_root = Path(result_folder_path)
		return
	...
	def test_next_payload(self):
		...
		try:
			if self._monitor_pool is not None:
				# We point the pool at the *actual* results root you already use
				# (e.g., ./cdns_test_res, ./recursive_test_res, or a custom --res_folder).
				base_dir = Path(result_folder_path)
				# The pool will route each resolver's events to its own monitor
				stream_round_to_monitor(
					mode=self.mode,
					unit_no=self.unit_num,
					round_no=self.round_num,
					base_dir=base_dir,
					pool=self._monitor_pool
				)
		except Exception as e:
			print(f"[monitor_bridge] warning: failed to stream predicates for unit {self.unit_num} round {self.round_num}: {e}")
	...
	def test_payloads(self):
		...
		# after while loop (unit finished)
		if hasattr(self, "_monitor_pool") and self._monitor_pool is not None:
			try:
				self._monitor_pool.end_all_sessions()
				self._monitor_pool.terminate_all()
			except Exception:
				pass
